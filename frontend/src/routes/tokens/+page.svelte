<script lang="ts">
	...
	$effect(() => {
		...
		if (auth.authEnabled && !auth.isAuthenticated) {
			goto(`/login?redirect=${encodeURIComponent('/tokens')}`);
			return;
		}
	...
	async function loadTokens(targetPage: number) {
		...
		try {
			...
			const response = await fetch(`/api/tokens?limit=${perPage}&offset=${offset}`);
	...
	const heading = $derived(auth.authEnabled ? 'Your active tokens' : 'Active webhook tokens');
	const description = $derived(
		auth.authEnabled
			? 'Owned hooks and hooks shared with you.'
			: 'Every active webhook token in this HookWatch instance.'
	);
</script>

<svelte:head>
	<title>HookWatch | Tokens</title>
</svelte:head>
...
<div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-10">
	<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
		...
		<div class="flex gap-2">
			<Button href="/" variant="secondary" size="sm">Create webhook</Button>
			{#if auth.isAdmin}
				...
				<Card class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
					<div class="min-w-0 flex-1">
						<div class="flex flex-wrap items-center gap-2">
							...
							{#if token.access_role}
								<Badge tone={token.access_role === 'owner' ? 'accent' : 'outline'}>{token.access_role}</Badge>
							{/if}
